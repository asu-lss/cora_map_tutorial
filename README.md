...
2. If you're new to healpix, read about it at https://healpix.sourceforge.io. (Optionally, for more details, see https://healpix.sourceforge.io/pdf/intro.pdf.)
...
4. Install `cora` (https://github.com/radiocosmology/cora) and `draco` (https://github.com/radiocosmology/draco) into your virtual environment. The recommended way to do this is to use the `mkchimeenv` script (https://github.com/chime-experiment/mkchimeenv), with the `--non-chime-member` if you are not a CHIME member, to create a new virtual environment with all CHIME packages and dependences included. Alternatively, you could clone `cora` and `draco` from their repositories and pip-install them from source (`pip install -e .` from within the directory for each package).
